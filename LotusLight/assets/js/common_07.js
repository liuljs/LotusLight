...
$(document).ready(function () {
    ...
    var donateArea = lightBox.find('.donateArea');
    var donateBranchz = lightBox.find('.donateBranchz');
    var limitAmount = '', swipeAmount = 6000;
    function stagingLimits() {
        if ($(this).val() >= swipeAmount) {
            stagingBlock.html('').removeClass('active').append(`
    ...
    donateMethods.on('change', function () {
        ...
        if ($(this).val() == 01) {
            if (donateAmount.val() >= swipeAmount) {
                stagingBlock.html('').removeClass('active').append(`
                    <label for="stagingMethods">分期方式</label>
                    <div>
                        <select class="stagingMethods" id="stagingMethods">
            ...
            donateAmount.on('change', stagingLimits);
        } else {
            stagingBlock.html('').addClass('active');
            donateAmount.unbind('change');
        }
        if ($(this).val() == 4 || $(this).val() == 6 || $(this).val() == 9 || $(this).val() == 10) {
            limitAmount = 20000;
            donateAmount.attr('max', limitAmount).attr('placeholder', '單次金額最低 NT$100；超商繳款上限 20,000 元');
        } else {
            limitAmount = '';
    ...
    // 依贈予地區決定贈予的單位
    donateArea.on('change', function () {
        $('.donateBranch option[value="preset"]').prop('selected', true);
        $('.donateBranch option[value="preset"]').change();
        if ($(this).val() == "domestic") {
            donateBranchz.eq(0).addClass('active').siblings().removeClass('active');
        } else {
            donateBranchz.eq(1).addClass('active').siblings().removeClass('active');
        };
    });
    // 依捐款用途，選擇其他能填寫寫其他的原因
    ...
    //
    var relatedBranch = relatedPage.find('.relatedForms .relatedBranch');
    var relatedArea = relatedPage.find('.relatedForms .relatedArea');
    var relatedBranchz = relatedPage.find('.relatedForms .relatedBranchz');
    // relatedPicks 
    ...
    quantities.on('change', function () {
        ...
        // 依捐款方式顯示分期
        if (relatedMethods.val() == 01) {
            if (totalAmount >= swipeAmount) {
    ...
    // 
    relatedArea.on('change', function () {
        $('.relatedBranch option[value="preset"]').prop('selected', true);
        $('.relatedBranch option[value="preset"]').change();
        if ($(this).val() == "domestic") {
            relatedBranchz.eq(0).addClass('active').siblings().removeClass('active');
        } else {
            relatedBranchz.eq(1).addClass('active').siblings().removeClass('active');
        };
    });
    //
    relatedBranch.on('change', function () {
        if ($(this).val() == "1" || $(this).val() == "4" || $(this).val() == "5" || $(this).val() == "8") {
            relatedPicks.html('').append(`
                <option value="0" selected>取貨方式</option>
                <optgroup label="寄送（選擇寄送方式及填寫備註）">
                    <option value="1">新竹物流寄送(運費100元)</option>
                    <option value="2">超商店到店(運費60元)</option>
                </optgroup>
                <optgroup label="面交（選擇地點及電話聯繫約定時間）">
                    <option value="4">各分會的辦公室面交</option>
                </optgroup>
            `);
        } else {
            relatedPicks.html('').append(`
               ...
               <optgroup label="面交（選擇地點及電話聯繫約定時間）">
                   <option value="3">約定時間至台灣雷藏寺面交</option>
                   <option value="4">各分會的辦公室面交</option>
               </optgroup>
            `);
        }
    ...
    function checkForm() {
        ...
        if (name.val() == '') {
            name.focus();
            return check = false, errorTxt = "請您填寫真實姓名！";
        }
        if (phone.val() == '') {
            phone.focus();
            return check = false, errorTxt = "請您填寫手機號碼！";
        } else if (!c.re_cel.test(phone.val())) {
            phone.focus();
            return check = false, errorTxt = "請填寫正確的手機格式09XX000000(共10碼)！";
        }
        /*if (email.val() == '') {
        ...
        if (email.val().length > 0 && !c.re_eml.test(email.val())) {
            email.focus();
            return check = false, errorTxt = "請填寫正確的 E-mail 格式！";
        }
        if (content.val() == '') {
            content.focus();
            return check = false, errorTxt = "請您填寫您想詢問的內容！";
        }
    ...
    btnSubmit.click(() => {
        ...
        if (check == true) {
            ...
            xhr.open("POST", "http://www.lotuslight.org.tw/Act/SendMailToBranch", true);
            xhr.onload = function () {
